...
npm run serve   # plain static server, no build step
npm run check   # syntax-check every .mjs/functions/api/*.js file and every JSON data file
npm test        # data-shape + logic unit tests
```
...
- Run `npm run check && npm test` before opening a PR. CI runs the same two commands.
